...
export const onboardingService = {
  async isComplete(userId: string, role: UserRole) {
    ...
    const table = role === 'tenant' ? 'tenant_onboarding' : 'landlord_onboarding';
  ...
  async getPendingOnboardings() {
    ...
    const tenantRows = (tenants.data || []).map((r: any) => ({ ...r, role: 'tenant' as const }));
    const landlordRows = (landlords.data || []).map((r: any) => ({ ...r, role: 'landlord' as const }));
  ...
  async getVerificationStatus(userId: string, role: UserRole): Promise<VerificationStatus> {
  ...
  async uploadDocument(userId: string, role: OnboardingRole, documentType: OnboardingDocumentType, file: File) {
